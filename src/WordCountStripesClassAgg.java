...
public class WordCountStripesClassAgg {
    ...
    public static class StripesClassAggMapper extends Mapper<Object, Text, Text, MapWritable> {
        ...
        private int d;
        // Accumulates stripes across ALL map() calls
        private Map<String, Map<String, Integer>> allStripes = new HashMap<>();
        ...
        public void map(Object key, Text value, Context context)
            ...
            for (int i = 0; i < tokens.size(); i++) {
                ...
                Map<String, Integer> stripe = allStripes.computeIfAbsent(word, k -> new HashMap<>());
        ...
        // Emit ONCE per mapper task
        @Override
        protected void cleanup(Context context) throws IOException, InterruptedException {
            Text wordKey = new Text();
            for (Map.Entry<String, Map<String, Integer>> entry : allStripes.entrySet()) {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 4) {
            System.err.println("Usage: WordCountStripesClassAgg <input> <output> <distance> <top50file>");
            System.exit(-1);
        }
        ...
        Job job = Job.getInstance(conf, "stripes class-level agg d=" + args[2]);
        job.setJarByClass(WordCountStripesClassAgg.class);
        ...
        if (success) {
            System.out.println("=========================================");
            System.out.println("STRIPES CLASS-LEVEL AGG - JOB SUCCESSFUL");
