...
public class WordCountPairsClassAgg {
        ...
        public void map(Object key, Text value, Context context)
            ...
            for (int i = 0; i < tokens.size(); i++) {
                ...
                for (int j = i + 1; j < Math.min(i + d + 1, tokens.size()); j++) {
                    ...
                    pairCounts.merge(joint, 1, Integer::sum);
                }
            }
        }

        // Emit ONCE per mapper task after ALL lines are processed
        @Override
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 4) {
            System.err.println("Usage: WordCountPairsClassAgg <input> <output> <distance> <top50file>");
            System.exit(-1);
        }
        ...
        Job job = Job.getInstance(conf, "pairs class-level agg d=" + args[2]);
        job.setJarByClass(WordCountPairsClassAgg.class);
        ...
        if (success) {
            System.out.println("=========================================");
            System.out.println("PAIRS CLASS-LEVEL AGG - JOB SUCCESSFUL");
